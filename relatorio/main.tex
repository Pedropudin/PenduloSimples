...
\title[Pêndulo Simples]{Pêndulo Simples com Newton-Rhapson}
\subtitle{Resolvendo o pêndulo simples usando o método de Newton-Rhapson}
...
\subsection{Método de Newton-Rhapson}

\begin{frame}{Método de Newton-Rhapson}
    ...
    O método precisa de um chute inicial $x_0$, então ele itera criando novos valores de $x$ seguindo a equação \ref{eq:newton-rhapson-iter}.

    \begin{equation}
        x_{i+1} = x_i - \frac{f(x_i)}{f'(x_i)}
        \label{eq:newton-rhapson-iter}
    \end{equation}
\end{frame}
...
\begin{frame}{Algoritmo de Thomas}
    ...
    Primeiro iteramos a matriz dos coeficientes eliminando a diagonal inferior, isso é feito
\end{frame}

\section{Metodologia}

\subsection{Adaptando Newton-Rhapson para sistema de equações}

\begin{frame}{Newton-Rhapson para Sistema de Equações}
...
\begin{frame}{Newton-Rhapson para Sistema de Equações}
    ...
    \begin{table}[]
        ...
        \begin{tabular}{|c|c|}
            ...
            f' & $\mathbb R^{m\text{ x }m} = \partial f/\partial x$ \\
            \hline
        \end{tabular}
    \end{table}
...
\begin{frame}{Resolvendo o Pêndulo Simples como PVC}
    Como vimos, o sistema pode ser descrito como a busca pela raiz de $G$, usando newton-rhapson teremos então que a iteração do sistema é dada pela equação \ref{eq:sistema-pendulo}
...
\begin{frame}{Resolvendo Sistema Linear}
    ...
    \begin{equation}
        \epsilon_r = \frac{\|\theta^{i+1} - \theta^i\|}{\|\theta^{i+1}\|}
...
\begin{frame}{Highlighting Information with Blocks}
  
  \begin{block}{Standard Block}
    This block adopts the presentation's primary blue theme. It is excellent for definitions, main theorems, or key takeaways.
  \end{block}

  \begin{alertblock}{Alert Block}
    This red block is designed to draw immediate attention to critical information, warnings, or crucial conditions.
  \end{alertblock}

  \begin{exampleblock}{Example Block}
    This green block is typically used to separate practical examples or proofs from the main theoretical content.
  \end{exampleblock}
